...
interface RuntimeFlagsN extends HKT {
  readonly [HKT.T]: RuntimeFlags;
}
 ...
 */
export interface RuntimeFlagsOps extends NewtypeIso<RuntimeFlagsN> {}
...
export function isEnabled(flags: RuntimeFlags, flag: RuntimeFlag): boolean {
  return (RuntimeFlags.reverseGet(flags) & flag) !== 0;
}
...
export function diff(oldValue: RuntimeFlags, newValue: RuntimeFlags) {
  return RuntimeFlags.Patch(oldValue.reverseGet ^ newValue.reverseGet, newValue.reverseGet);
}
