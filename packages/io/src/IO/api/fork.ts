...
export function fork<R, E, A>(ma: IO<R, E, A>, __tsplusTrace?: string): URIO<R, FiberRuntime<E, A>> {
  return IO.withFiberRuntime((fiberState, status) =>
    IO.succeedNow(unsafeFork(ma, fiberState, status.runtimeFlags, __tsplusTrace)),
  );
}
...
export function unsafeFork<R, E, A, E1, B>(
  ...
  const fiber = unsafeForkUnstarted(effect, parentFiber, parentRuntimeFlags, trace);
  fiber.start(effect);
  return fiber;
}

export function unsafeForkUnstarted<R, E, A, E1, B>(
  effect: IO<R, E, A>,
  parentFiber: FiberRuntime<E1, B>,
  parentRuntimeFlags: RuntimeFlags,
  trace?: string,
  ...
  const parentScope = parentFiber.getFiberRef(FiberRef.forkScopeOverride).getOrElse(parentFiber.scope);
  parentScope.unsafeAdd(childFiber as FiberRuntime<any, any>);
