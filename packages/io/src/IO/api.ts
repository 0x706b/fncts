...
import type { Running } from "../FiberStatus";
import type { RuntimeFlags } from "../RuntimeFlags.js";
import type * as P from "@fncts/base/typeclass";
import type { _E, _R } from "@fncts/base/types";
import type { FiberDescriptor } from "@fncts/io/FiberDescriptor";
import type { Canceler } from "@fncts/io/IO/definition";

import { identity, pipe, tuple } from "@fncts/base/data/function";
import { YieldNow } from "@fncts/io/IO/definition";
import { Sync } from "@fncts/io/IO/definition";
import { OnSuccessAndFailure } from "@fncts/io/IO/definition";
import { OnSuccess } from "@fncts/io/IO/definition";
import { GenerateStackTrace } from "@fncts/io/IO/definition";
import { UpdateRuntimeFlags } from "@fncts/io/IO/definition";
import { Stateful } from "@fncts/io/IO/definition";
import { Async, Fail, IO, IOError, SucceedNow } from "@fncts/io/IO/definition";
 ...
 */
export function as_<R, E, A, B>(self: IO<R, E, A>, b: Lazy<B>, __tsplusTrace?: string): IO<R, E, B> {
  return self.map(() => b());
}
 ...
 */
export function flatMapError_<R, R1, E, E1, A>(
  ma: IO<R, E, A>,
...
export function descriptorWith<R, E, A>(f: (d: FiberDescriptor) => IO<R, E, A>, __tsplusTrace?: string): IO<R, E, A> {
  return IO.withFiberRuntime((state, status) =>
    f({
      id: state.id,
      status,
      interruptors: state.getFiberRef(FiberRef.interruptedCause).interruptors,
    }),
  );
}
 ...
 */
export function getOrFailWith_<E, A>(maybe: Maybe<A>, onNothing: Lazy<E>, __tsplusTrace?: string): FIO<E, A> {
 ...
 */
export function orElseFail_<R, E, A, E1>(ma: IO<R, E, A>, e: Lazy<E1>, __tsplusTrace?: string): IO<R, E1, A> {
 ...
 */
export function parallelErrors<R, E, A>(io: IO<R, E, A>, __tsplusTrace?: string): IO<R, List<E>, A> {
 ...
 */
export function repeatWhile_<R, E, A>(ma: IO<R, E, A>, f: (a: A) => boolean, __tsplusTrace?: string): IO<R, E, A> {
...
//   __tsplusTrace?: string,
// ): IO<R, E, A> {
//   return FiberRef.currentSupervisor.locallyWith()
