...
export abstract class Conc<A> implements Iterable<A>, Hashable, Equatable {
  ...
  [Symbol.equatable](that: unknown): boolean {
    return (
      Conc.isConc(that) &&
      (this as Conc<A>).corresponds(that, Equatable.strictEquals)
    );
  }
}
...
abstract class ConcImplementation<A> extends Conc<A> implements Iterable<A> {
  ...
  concat<B>(that: ConcImplementation<B>): ConcImplementation<A | B> {
    ...
    if (that._concTag === ConcTag.PrependN) {
      const conc = fromArray(
        (that.buffer as Array<A>).takeLast(that.bufferUsed)
      );
  ...
  update<A1>(index: number, a1: A1): ConcImplementation<A | A1> {
    ...
    const bufferValues  = binary
      ? alloc(UPDATE_BUFFER_SIZE)
      : Array.alloc<any>(UPDATE_BUFFER_SIZE);
...
class AppendN<A> extends ConcImplementation<A> {
  ...
  [Symbol.iterator](): Iterator<A> {
    return (this.start as Iterable<A>)
      .concat((this.buffer as Array<A>).take(this.bufferUsed))
      [Symbol.iterator]();
  }
  ...
  append<A1>(a: A1): ConcImplementation<A | A1> {
    ...
    if (this.bufferUsed < this.buffer.length) {
      ...
      const buffer =
        this.binary && binary ? alloc(BUFFER_SIZE) : Array.alloc(BUFFER_SIZE);
      buffer[0]  = a;
      const conc = fromArray(this.buffer as Array<A>).take(this.bufferUsed);
      return new AppendN<A | A1>(
...
class PrependN<A> extends ConcImplementation<A> {
  ...
  [Symbol.iterator](): Iterator<A> {
    return ((this.buffer as Array<A>).take(this.bufferUsed) as Iterable<A>)
      .concat(this.end)
      [Symbol.iterator]();
  }
  ...
  forEach<B>(startIndex: number, f: (i: number, a: A) => B): void {
    for (
      let i = BUFFER_SIZE - this.bufferUsed, j = 0;
      i < BUFFER_SIZE;
      i++, j++
    ) {
...
class Update<A> extends ConcImplementation<A> {
  ...
  update<A1>(i: number, a: A1): ConcImplementation<A | A1> {
    ...
    if (this.used < UPDATE_BUFFER_SIZE) {
      ...
      const bufferIndices = Array.alloc<number>(UPDATE_BUFFER_SIZE);
      const bufferValues  = this.binary && binary
          ? alloc(UPDATE_BUFFER_SIZE)
          : Array.alloc<any>(UPDATE_BUFFER_SIZE);
      bufferIndices[0] = i;
      bufferValues[0]  = a;
...
function copyArray<A>(
  source: ArrayLike<A>,
  sourcePos: number,
  dest: MutableArray<A> | Uint8Array,
  destPos: number,
  length: number
): void {
...
export function fromArray<A>(array: ArrayLike<A>): ConcImplementation<A> {
  if (array.length === 0) {
    ...
    return "buffer" in array
      ? (new ByteChunk(array as any) as any)
      : new Chunk(Array.from(array));
  }
}
...
export function isConc(u: unknown): u is Conc<unknown> {
  return (
    isObject(u) &&
    "_typeId" in u &&
    u["_typeId" as keyof typeof u] === ConcTypeId
  );
}
