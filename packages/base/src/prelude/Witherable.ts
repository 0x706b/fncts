 ...
 */
export interface Witherable<F extends HKT, FC = HKT.None>
  extends Filterable<F, FC>,
    Traversable<F, FC> {
 ...
 */
export function mkWitherable<F extends HKT, FC = HKT.None>(
  F: WitherableMin<F, FC>,
): Witherable<F, FC> {
  return HKT.instance({
    ...Filterable(F),
    ...Traversable(F),
...
export function filterAF_<F extends HKT, CF = HKT.None>(F: WitherableMin<F, CF>): filterA_<F, CF> {
  return (G) => (fa, p) =>
    F.wither_(G)(fa, (a) => G.map_(p(a), (bb) => (bb ? Just(a) : Nothing())));
}
