  ...
  return HKT.instance({
    ...
    wilt: <G extends HKT, CG = HKT.None>(A: Applicative<G, CG>) => {
      ...
      return <KG, QG, WG, XG, IG, SG, RG, EG, A, B, B1>(
        ...
        <K, Q, W, X, I, S, R, E>(
          wa: HKT.Kind<F, FC, K, Q, W, X, I, S, R, E, A>
        ) =>
          wilt_(wa, f);
    },
    wither_: F.wither_,
...
export interface witherSelf<F extends HKT, FC = HKT.None> {
  <KF, QF, WF, XF, IF, SF, RF, EF, A>(
    self: HKT.Kind<F, FC, KF, QF, WF, XF, IF, SF, RF, EF, A>
...
export function filterAF_<F extends HKT, CF = HKT.None>(
  F: WitherableMin<F, CF>
): filterA_<F, CF> {
  return (G) => (fa, p) =>
...
export function filterAF<F extends HKT, CF = HKT.None>(
  F: WitherableMin<F, CF>
): filterA<F, CF> {
