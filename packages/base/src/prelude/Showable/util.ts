...
export function getKeys(value: object, showHidden = true): Array<PropertyKey> {
  ...
  if (!showHidden) {
    return keys.filter((k) => Object.prototype.propertyIsEnumerable.call(value, k));
  }
  return keys;
}
...
export function getConstructorName(value: object): [string | null, Conc<PropertyKey>] {
  ...
  const tmp      = obj;
  let firstProto;
  let protoProps = Conc.empty<PropertyKey>();
  while (obj || isUndetectableObject(obj)) {
    ...
    if (
      ...
      if (firstProto !== obj || !builtInObjects.has(descriptor.value.name)) {
        protoProps = protoProps.concat(getPrototypeProperties(tmp, firstProto || tmp));
      }
