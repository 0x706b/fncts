...
export type WitherableWithIndexMin<
  F extends HKT,
  FC = HKT.None
> = FilterableWithIndexMin<F, FC> &
...
export function filterWithIndexAF_<F extends HKT, CF = HKT.None>(
  ...
  return (G) => (fa, p) =>
    F.witherWithIndex_(G)(fa, (i, a) =>
      G.map_(p(i, a), (bb) => (bb ? Just(a) : Nothing()))
    );
}
