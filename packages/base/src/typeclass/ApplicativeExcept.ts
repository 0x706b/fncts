...
export interface ApplicativeExcept<F extends HKT, FC = HKT.None> extends Applicative<F, FC>, Fail<F, FC> {
  catchAll: <K, Q, W, X, I, S, R, E, K1, Q1, W1, X1, I1, S1, R1, E1, A1>(
    f: (
      e: E,
    ) => HKT.Kind<
      F,
...
): <K, Q, W, X, I, S, R, E, K1, Q1, W1, X1, I1, S1, R1, E1, A1>(
  f: (
    e: E,
  ) => Maybe<
    HKT.Kind<
...
> {
  // @ts-expect-error
  return (f) => F.catchAll((e) => f(e).match(() => F.fail(e), identity));
}
...
export function either<F>(
  ...
  return (self) =>
    pipe(
      self,
      F.map(Either.right),
      F.catchAll((e) => F.pure(Either.left(e))),
    );
}
