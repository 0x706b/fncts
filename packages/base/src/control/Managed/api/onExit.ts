 ...
 */
export function onExit_<R, E, A, R1>(ma: Managed<R, E, A>, cleanup: (exit: Exit<E, A>) => IO<R1, never, unknown>): Managed<R & R1, E, A> {
  return new Managed(
            ...
            Finalizer.get(
              (exit) =>
                innerReleaseMap
                  ...
                  .result.zipWith(cleanup(exitEA).provideEnvironment(r1).result, (l, r) => IO.fromExitNow(l.apSecond(r))).flatten,
            ),
          ),
        );
