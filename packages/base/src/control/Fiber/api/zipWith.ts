 ...
 */
export function zipWith_<E, A, E1, B, C>(self: Fiber<E, A>, that: Fiber<E1, B>, f: (a: A, b: B) => C): Fiber.Synthetic<E | E1, C> {
  return {
    _tag: "SyntheticFiber",
    ...
    interruptAs: (id) => self.interruptAs(id).zipWith(that.interruptAs(id), (ea, eb) => ea.zipWithCause(eb, f, Cause.both)),
