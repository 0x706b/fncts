    ...
    interruptAs: (fiberId) =>
      pipe(
        IO.foreach(fibers, (f) => f.interruptAs(fiberId)).map((exits) =>
          exits.foldRight(
            Exit.succeed(Conc.empty<A>()) as Exit<E, Conc<A>>,
            (a, b) => a.zipWithCause(b, (a, as) => as.prepend(a), Cause.both)
          )
        )
      ),
    poll: IO.foreach(fibers, (f) => f.poll).map((exits) =>
      exits.foldRight(
        Just(Exit.succeed(Conc.empty()) as Exit<E, Conc<A>>),
        (a, b) =>
          a.match(
            () => Nothing(),
            (ra) =>
              b.match(
                ...
                (rb) =>
                  Just(
                    ra.zipWithCause(rb, (a, as) => as.prepend(a), Cause.both)
                  )
              )
          )
      )
