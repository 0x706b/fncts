...
export class FilterInputIO<
  ...
  R2,
  E2
> extends QueueInternal<RA & R2, RB, EA | E2, EB, A1, B> {
  constructor(
  ...
  offer(a: A1): IO<RA & R2, EA | E2, boolean> {
    return this.f(a).chain((b) =>
      b ? this.queue.offer(a) : IO.succeedNow(false)
    );
  }

  offerAll(as: Iterable<A1>): IO<RA & R2, EA | E2, boolean> {
    return IO.foreach(as, (a) =>
      this.f(a).map((b) => (b ? Just(a) : Nothing()))
    ).chain((ms) => {
